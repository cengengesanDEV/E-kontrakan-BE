...
const getHistoryCustomer = (status,id) => {
    return new Promise((resolve, reject) => {
        const query =
          "select tr.id,de.tipe_kontrakan,(select image from image_kontrakan where de.id = id_detail_kontrakan limit 1) as image_kontrakan,(select full_name from users as us inner join category_kontrakan as ca on ca.id_user = us.id inner join detail_kontrakan as de on de.id_kontrakan = ca.id inner join transaction as tr on tr.id_kontrakan = de.id limit 1) as owner,us.full_name as customer,tr.checkin,tr.checkout,tr.status_booking,tr.order_date,tr.total_price,tr.payment_method from transaction tr inner join detail_kontrakan as de on de.id = tr.id_kontrakan inner join users as us on us.id = tr.id_users where us.id = $1 and tr.status_booking = $2 and tr.deleted_at is null";
...
const getTransactionsByStatus_booking = (status, id) => {
  return new Promise((resolve, reject) => {
    const query =
      "select tr.id,de.tipe_kontrakan,(select full_name from users where tr.id_users = id) as customers,us.full_name as owner,tr.checkin,tr.checkout,tr.status_booking,tr.order_date,tr.total_price,tr.payment_method from transaction tr inner join detail_kontrakan as de on de.id = tr.id_kontrakan inner join category_kontrakan as ca on ca.id = de.id_kontrakan inner join users as us on us.id = ca.id_user where us.id = $1 and tr.status_booking = $2 and tr.deleted_at_owner is null";
    postgreDb.query(query, [id, status], (err, result) => {
      if (err) {
        console.log(err);
        return reject({ status: 500, msg: "internal server error" });
...
const finishOrder = (id) => {
  return new Promise((resolve, reject) => {
    const query =
      "update transaction set status_boking = 'done' where id = $1 returning id_kontrakan ";
    postgreDb.query(query, [id], (err, result) => {
      if (err) {
        console.log(err);
        return reject({ status: 500, msg: "internal server error" });
