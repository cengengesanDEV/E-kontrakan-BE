...
const postDetail = (req) => {
  return new Promise((resolve, reject) => {
    const { id_kontrakan, tipe_kontrakan, fasilitas, price, deskripsi } =
      req.body;
      const fasilitasParse = JSON.parse(fasilitas)
    const timeStamp = Date.now() / 1000;
    const images = req.file;
    console.log(fasilitas);
    const query = `insert into detail_kontrakan(id_kontrakan,tipe_kontrakan,fasilitas,price,deskripsi,created_at,updated_at) values($1,$2,ARRAY ${fasilitasParse},$3,$4,to_timestamp($5),to_timestamp($6)) returning *`;
    postgreDb.query(
      query,
